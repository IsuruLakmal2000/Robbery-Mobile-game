...
public class BillionareClubController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (PlayerPrefs.GetInt("is_unlock_billionare_club", 0) == 1)
        {
            ...
            {
                lastCollectTime = DateTime.Now; // If no previous record, just set the current time
            }
        ...
        else
        {
            buyBtn.onClick.AddListener(() =>
            {
                ...
                if (currentMoney >= 10)
                {
                    ...
                    profitAmount.text = currentMoney.ToString();
        ...
        // Update every 10 seconds instead of every second
        //  InvokeRepeating(nameof(UpdateEarnings), 10f, 10f); // Update every 10 seconds
        // UpdateUI();
    }
    ...
    void UpdateEarnings()
    {
        ...
        Debug.Log("total sec from last - " + secondsToCalculate);
        int newEarnings = secondsToCalculate * EARN_PER_SECOND;
        Debug.Log("new earn profit  - " + newEarnings);
        // Add the new earnings, but do not exceed the daily limit
